...
(def (main . args)
  (parameterize ((program-name "sort"))
    (call-with-getopt
      (lambda (opt)
        (let-hash opt
          (let ((files .rest)
                ...
                (ignore-nonprinting? .?ignore-nonprinting)
                (month? .?month)
                (stable? .?stable)
                (check? .?check)
                (output .?output)
            ...
            (let ((files (if (null? files) '("-") files))
              ...
              (let ((lines (read-all-lines files zero-term?)))
                (if check?
      ...
      (flag 'human-numeric "-h" "--human-numeric-sort"
...
(def (read-all-lines files zero-term?)
