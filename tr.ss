    ...
    (let loop ((args args) (complement? #f) (delete? #f) (squeeze? #f))
      (cond
        ...
        ((and (pair? args) (member (car args) '("--version")))
         (display "tr (gerbil-coreutils) 0.1.0\n"))
        (else
        ...
        (let (c (string-ref str i))
          (cond
            ...
            ((and (< (+ i 2) len)
             ...
             (let ((start-c (car acc))  ;; Previous char is range start
                   (end-c (string-ref str (+ i 2))))
               ;; Remove the start char from acc (already added) and add the full range
               (let ((range-chars (char-range start-c end-c)))
                 (loop (+ i 3) (append (reverse (cdr range-chars)) (cdr acc))))))
            (else
             (loop (+ i 1) (cons c acc)))))))))
  ...
  (let ((table (make-vector 256 #f)))
    (if complement?
      ...
      (let ((set1 (list->char-set chars1))
            (last-c2 (if (null? chars2) #f (last chars2))))
        (let loop ((i 0))
          (when (< i 256)
